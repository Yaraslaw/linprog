/*  Linprog: Linear and Mixed-Integer Programming Library for SWI-Prolog
    ...
    Linprogr is a SWI-Prolog library providing support for Linear Programming
...
/* No output for linprogr attributes */
...
:- use_module(library(clpr), [{}/1 as cons]).
...
{Bound} :- 
    ...
    call(cons(Bound)), % Calling CLP(R) to check satisfiability of the constraint
