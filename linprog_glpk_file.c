...
const double inf = 1e5; // infinity TODO: remove usage of inf
...
// ------------------------------------- Reading Cons and counting #variables and #rows -------------------------------
    ...
    while (PL_get_list(tail, head, tail)) {
        ...
        int djn_num = 1; // number of disjunctions variables in the constraint
        for (int i = 0; i < len; ++i) {
            if (s[i] == '=' || s[i] == '>') {
                if (s[i] == '>') {
                    ++i; // LO
                ...
                else { // FX need to be splited into two parts
                    ++row_number;
                    ar_size += cons_counter;
                } 
            } else if (s[i] == '_') {
                // Find a variable!
                ++ar_size; 
        ...
        ++row_number;
        ar_size += djn_num; // sum(D_i) <= X

    }
    ar_size += row_number; // in any constraint add djn_var. 
...
/* This one has to be independent reading part */
    ...
    while (PL_get_list(tail, head, tail)) {
        ...
        ++maxVarId; /* each row of cons has at least one D_i*/
        ++row_number; /* For each djn part, we have to create a constraint: sum(D_i) >= 1 */
        for (int i = 0; i < len; ++i) {
            if (s[i] == ';')
                ++maxVarId; // If there is a djn, increase number of D_i
        }
    }
    --maxVarId; // maxVarId should point at the last used index  
...
// ------------------------------------- Initial part of GLPK ----------------------TODO : check ar_size -------------------------------
    ...
    /* Setting the bounds for the disjunction variables */
    for (int i = djn_first; i <= maxVarId; ++i) {
...
// ------------------------------------- Going through Cons and Add them to GLPK --------------------------------------
    ...
    int djn_id = djn_first; // iterator through disjunction variables
    tail = PL_copy_term_ref(cons);
    while (PL_get_list(tail, head, tail)) {
        int djn_start = djn_id; 
        ...
        bool isSecondTimeFX = false; // flag for the second time of the FX constraint
        int first_cons_pos = 0; // position of the first symbol of the constraint in the string s
        if (!PL_get_string(head, &s, &len)) {
            printf("non-string element in CONS was found\n");
            continue;
        }        
	// printf("Next station is : %s\n", s), fflush(stdout);
		...
		// printf("Id is %d\n", id), fflush(stdout);
            ...
            /* If for constraints ending */
                ...
                // if the sign is LO, LOS, or FX on its first time
                if (flag == GLP_LO || (flag == GLP_FX && !isSecondTimeFX) || flag == -GLP_LO) {
                    ia[last_symbol] = row_id;
                    ja[last_symbol] = djn_id;
                    ar[last_symbol] = inf;
                    ++last_symbol;
                }
                // if the sign is UP, UPS, or FX on its second time
                if (flag == GLP_UP || (flag == GLP_FX && isSecondTimeFX) || flag == -GLP_UP) {
                    ia[last_symbol] = row_id;
                    ja[last_symbol] = djn_id;
                    ar[last_symbol] = -inf;
                    ++last_symbol;
                ...
                if (flag == GLP_FX) {
                    if (!isSecondTimeFX) {
                        glp_set_row_bnds(lp, row_id, GLP_LO, bound, bound);
                        isSecondTimeFX = true;

                        /* Init after calling adding_FX_constraint_second_time */
                        i = first_cons_pos - 1; // Because of i++ (going second time)
                        /* Next row, reset flags */
                        ++row_id;
                        negative = false;
                        flag = 42;
                        bound = 0;
			...
			continue;
                        ...
                        // printf("Bound was set to[FX:UP] %lf\n", bound);
                        glp_set_row_bnds(lp, row_id, GLP_UP, bound, bound);
                        isSecondTimeFX = false;
                    }
                } else if (flag < 0) { // if the sign is strict 
                    if(flag == -GLP_LO) {
                ...
                first_cons_pos = i + 1; // set the position of the first symbol of the next constraint
                if (s[i] == ';') {
                    ++djn_id;
                }
            } else if (s[i] != ' ' && s[i] != '\'' && s[i] != '\"') {
        ...
        // setting a new constraint for the disjunction variables
        for (int j = djn_start; j <= djn_id; ++j) {
            ia[last_symbol] = row_id;
            ja[last_symbol] = j;
            ar[last_symbol] = 1.0;
            ++last_symbol;
        }
        glp_set_row_bnds(lp, row_id, GLP_UP, djn_id - djn_start, djn_id - djn_start);
        ++row_id;
        ++djn_id;
	// printf("Row Added\n\n"), fflush(stdout);
    }
...
// ------------------------------------- Setting Ints kind, Obj adding ------------------------------------------------
    ...
    // TODO: proof if this for is needed 
    for (int i = 1; i < djn_first; ++i) {
...
// ------------------------------------- Simplex part -----------------------------------------------------------------
    ...
    parm.tol_bnd = 1e-9; // tolerance for the bounds
    parm.tol_piv = 1e-9; // tolerance for the pivots
