...
assert_vars_types((C & F),VN) :-
  ...
  (\+ type(Var,_),!,           % Var hasn't been declared yet
   ...
   (has_finite(Type) ->
      assert_fintype(Type),
      b_getval(finite_vars,FV),
      nb_setval(finite_vars,[[Var,Type] | FV])
   ;
      true
   ),
   ((Type = sum(_),! ; Type = enum(_)) ->
      b_getval(sum_vars,SV),
      nb_setval(sum_vars,[[Var,Type] | SV])
   ;
      true
   ),
   (b_getval(curr_clause,H/A),!,
    b_getval(typed_vars,TV),
    nb_setval(typed_vars,[[H/A,Var,Type]|TV])
   ;
    true
   ),
   assert_vars_types(F,VN)
...
assert_vars_types((C & _),_) :-
  C =.. [H|_],
  member(H,[def_type,dec_p_type,dec_pp_type]),!,   % reserved words
...
assert_vars_types(A,VN) :- 
  ...
  (\+ type(Var,_),!,          % Var hasn't been declared yet
   ...
   (b_getval(curr_clause,H/Ar),!,
    b_getval(typed_vars,TV),
    nb_setval(typed_vars,[[H/Ar,Var,Type]|TV])
   ;
    true
   )
   ;
   type(Var,Type),!           % if already declared but types coincide, do nothing
   ;
   print_type_error_dec_3(dec(V,Type),VN)
  ).
...
%
% (2) in foreach(X in Y,...) variables in X needn't be declared.
%     a simple type inference rule is implemented. first the type of
%     Y is computed which must be a set type; then each variable in
%     X is typed from Y's type and X's form; in that case the type
%     of each variable in X is asserted
%
% TODO same as in (2) can be applied to RIS
%
typecheck_constraint(foreach(D,F),VN) :- !,
  typecheck_constraint(foreach(D,[],F,true),VN).
typecheck_constraint(foreach(X in Y,P,F1,F2),VN) :- !,
  typecheck_term(Y,Ty,VN),!,    % (2)
  (Ty = set(Tx),!,
   term_variables(X,CtrlVars),
   (forall(member(B,CtrlVars),
     (get_type_from_term(B,X,Tx,Tb), get_var(VN,B,Var), assertz(type(Var,Tb)))),!
   ;
    print_type_error_foreach2(X in Y,VN,Tx)
   ),
   setlog:conj_append(F1,F2,F),
   typecheck_clause(F,VN),
   term_variables([X,P],LocVars),
   forall(member(A,LocVars),(get_var(VN,A,V),retract(type(V,_)),! ; true))  % (1)
  ;
   print_type_error_foreach1(X in Y,VN,Ty)
  ).
typecheck_constraint(foreach([X|Y],P,F1,F2),VN) :- !,
  setlog:unfold_nested_foreach(foreach([X|Y],P,F1,F2),FEflat),
  typecheck_clause(FEflat,VN).
...
typecheck_constraint(let(P,D,F),VN) :-
  ...
  forall(member(A,P),(get_var(VN,A,V),retract(type(V,_)),! ; true)).
% special case for TTF
typecheck_constraint(let(P,D,F),VN) :-
  P = [Y],
  D = ran(R,Y),!,
  typecheck_term(R,set([_,Try]),VN),
  get_var(VN,Y,Var), assertz(type(Var,set(Try))),
  setlog:conj_append(D,F,NF),
  typecheck_clause(NF,VN),
  retract(type(Var,_)).
% normal case
...
typecheck_constraint(nlet(P,D,F),VN) :- typecheck_constraint(let(P,D,F),VN).
...
typecheck_term(S,set(T),VN) :-
  ...
  typecheck_term(X,T1,VN),
  (T1 = set(T),!
  ;
   print_type_error_extset(S,VN)
  ).
typecheck_term(S,set(T),VN) :-
  S = {}(_),!,
  setlog:set_preproc(S,SW,_),
  typecheck_term(SW,set(T),VN).
...
%                non-nullary constructor x(T) -> x(c) where c:T
...
print_type_error_foreach1(X in Y,VN,Ty) :-
   term_string(X in Y,S1,[variable_names(VN)]),
   term_string(Ty,S3),
   string_concat("type error: in ",S1,C1),
   string_concat(C1,"\n\t",C11),
   string_concat(C11,S3,C2),
   string_concat(C2," isn't a set type",C3),
   throw(setlog_excpt(C3)).

print_type_error_foreach2(X in Y,VN,Ty) :-
   term_string(X in Y,S1,[variable_names(VN)]),
   term_string(X,S2,[variable_names(VN)]),
   term_string(Ty,S3),
   string_concat("type error: in ",S1,C1),
   string_concat(C1,"\n\t",C11),
   string_concat(C11,S2,C2),
   string_concat(C2," can't be typed with ",C3),
   string_concat(C3,S3,C4),
   throw(setlog_excpt(C4)).

print_type_error_extset(Y,VN) :-
   setlog:with_postproc(Y,YS),
   term_string(YS,S1,[variable_names(VN)]),
   string_concat("type error: ",S1,C1),
   string_concat(C1," can't be typed",C11),
   throw(setlog_excpt(C11)).
