...
:- dynamic(ttf/0).            %ttf file: loaded
:- dynamic(help_on/0).        %help file: loaded
...
:- multifile(setlog_command/1).     %in this file and in files 'setlog_tc.pl', 'setlog_ttf.pl' and 'setlog_vcg.pl' 
:- multifile(ssolve_command/1).     %in this file and in files 'setlog_tc.pl', 'setlog_ttf.pl' and 'setlog_vcg.pl'
...
set_default :-
    ...
    nb_setval(int_solver,IS),        % set default value for int_solver: clpq
    default_sel_strategy(S),    
    nb_setval(strategy,S),           % set default value for atom selection strategy: cfirst
    all_prover_mode_strategies(PMS),
    nb_setval(prover_all,[all,PMS]), % set default value for all prover mode strategies
    default_timeout(T),
    nb_setval(timeout,T),            % set default value for timeout
    nb_setval(timeout_finish,false). % last goal hasn't finished in timeout
set_default_control :-
    nb_setval(irules,on),            % set default value: irule 
    ...
    default_mode(M), 
    b_setval(smode,M),               % set default value for mode: prover([])
    nb_setval(fix_size,off),         % set default value: no fix_size
    nb_setval(groundsol,off),        % set default value: no groundsol
    nb_setval(show_min,off),         % set default value: no show_min
    nb_setval(type_check,off).       % set default value: no type_check
...
top_level :-
    ...
    catch(check_variable_names(VarNames),setlog_excpt(Msg),excpt_action(Msg)),
    ...
    skip_return,    
    term_variables(Constr,ConstrVars),
       % excludes some named singleton vars (also called anonymous
       % variables) from the computed answer
       % eg. in un({1},{a},_U) & un(_U,{[b,1]},A) _U isn't printed,
       % as in Prolog. If a named singleton var has some relation
       % with Constr is not excluded, it remains as a normal var
    exclude(ns_var_nin_constr(ConstrVars),VarNames,VarNames1),
       % exlude/3 in SWI standard library
    prepare_answer(Constr,VarNames1,VarNames_ext1,Constr1Ext,Vars),
    nl, write_subs_constr(VarNames_ext1,Constr1Ext,Vars,Goal1),
    top_level.                                                
top_level :-
    nl, write_no, nl,
    skip_return,
...
% checks that no variable name is of the form
% _N<number> or _X<number>
%
check_variable_names(VN) :-
  fresh_nl_variable_prefix([_|LNL]),    % take only letter code
  atom_codes(NL,LNL),
  fresh_loc_variable_prefix([_|LLoc]),  % take only letter code
  atom_codes(Loc,LLoc),
  check_variable_names1(VN,NL,Loc).

check_variable_names1(VN,NL,Loc) :-
   member(Name = _,VN),
   sub_atom(Name,0,1,_,'_'),            % first char is '_'    
   sub_atom(Name,1,1,_,L),              % second char
   member(L,[NL,Loc]),                  %            is non-local or local prefix
   sub_atom(Name,2,1,_,N),              % third char
   atom_number(N,_),!,                  %            is a number
   throw(setlog_excpt('variable names cannot begin with _N<number> or _X<number>')).
check_variable_names1(_,_,_).

% to be used by exclude/3     
% 
% (1) N is a named singleton var (or anonymous var),
%     that is a variable name beginning with _Q
%     where Q is an uppercase letter
%
ns_var_nin_constr(ConsVar,N = T) :-
  sub_atom(N,0,2,_,S), atom_concat('_',L,S), char_type(L,upper),   % (1)
  term_variables(T,TV),
  forall(member(X,TV),\+clpq:memberchk_eq(X,ConsVar)).      % no var in TV belongs to ConsVar
...
prepare_answer(Constr,VarNames,VarNames_ext1,Constr1Ext,Vars) :-  
    ...
    term_variables(VarNames_ext1,V1),
    term_variables(Constr1,V2),
    append(V1,V2,AllVars),
    append(Vars,ConstrVars,NLVars),
        % ord_subtract/3 in SWI standard library
        % LocVars --> local variables
    ord_subtract(AllVars,NLVars,LocVars),
    fresh_nl_variable_prefix(PrfNL),
    fresh_loc_variable_prefix(PrfLoc),
    rename_fresh_vars(Vars,PrfNL,1,N),
    rename_fresh_vars(ConstrVars,PrfNL,N,_),
    rename_fresh_vars(LocVars,PrfLoc,1,_).
...
rename_fresh_vars([],_,Num,Num).
rename_fresh_vars([X|R],Prf,Num,NumF) :-
    var(X),!,
    ...
    X=XNew,
    Num1 is Num + 1,
    rename_fresh_vars(R,Prf,Num1,NumF).
rename_fresh_vars([_X|R],Prf,Num,NumF) :-
    rename_fresh_vars(R,Prf,Num,NumF).
...
%
write_subs_constr(Subs,Constr,Vars,Goal) :-    
    (nb_getval(timeout_finish,true) -> 
         write_timeout, nl,
         nb_setval(timeout_finish,false)
    ; 
         write_subs_constr1(Subs,Constr,Vars,Goal)
    ).

write_subs_constr1([],[],_,_) :-
    !,
    write_yes, nl.
write_subs_constr1(Subs,Constr,Vars,Goal) :-   
    (Subs = [] ->
        nl,write('?????????? this case never occurs ??????????'),nl,
        %siccome i constraint in Constr contengono necessariamente
        %variabili, allora si ricade comunque nel caso Subs = [true|_];
        %se e' cosi', allora la var. Prn puo' essere eliminata
        %perche' ha sempre valore 'y'
        true
    ;
     Subs = [true|_] ->
        write_true, 
    ...
    ask_the_user(Constr,Vars,Prn,Goal).   

%ask_the_user(+Prn): if Prn is unbound do nothing; otherwise, 
%ask the user for another solution
%
ask_the_user(_,_,Prn,_) :- 
    var(Prn),!,
    nl,write('?????????? this case never occurs ??????????'),nl.
    %per quanto detto sopra, Prn ha sempre valore 'y' quindi
    %questo caso non si puo' mai verificare
ask_the_user(Constr,Vars,Prn,Goal) :-
    (one_sol_atom(Goal) ->
       true
    ;
...
% one_sol_atom(A): true if A is an atom of the form (_)!, or a setlog command,
% or one of a list of deterministic built-ins
%
one_sol_atom((_A)!) :- !.
one_sol_atom(A) :- 
    setlog_command(A),!.
one_sol_atom(A) :- 
    member(A,[write(_),assert(_),time(_,_)]).
...
write_subs_all([], _) :- !,
    nl,write('?????????? this case never occurs ??????????'),nl.
    %quando la write_subs_all viene richiamata dalla write_subs_constr1
    %il caso Subs = [] e' gia' stato trattato dalla prima clausola write_subs_all
    %(n.b. se Subs = [] allora necessariamente anche Constr = [])
    %quando invece la write_subs_all viene richiamata ricorsivamente 
    %la ricorsione si ferma ai casi Subs = [X=t] o Subs = [X=t,true,...]
    %e quindi non c'e' mai la chiamata ricorsiva write_subs_all([],...)
...
write_timeout :-
    (  current_prolog_flag(unix, true),!,
       ansi_format([bold,fg(255,255,50)],'timeout',[])    % yellow
     ;
       current_prolog_flag(windows, true),!,
       %ansi_format([bold,fg(255,255,50)],'timeout',[])   % yellow
       ansi_format([bold,fg(red)],'timeout',[])           % red   
    ).
...
% {log} facilities for set definition and manipulation,
...
%%%% Timeout (an integer constant specifying an amount of time in milliseconds
%%%% Res = success | time_out | maybe
%%%% to execute a {log} goal Goal with an output constraint list OutConstrList 
%%%% and a timeout Timeout, with a (possibly empty) list OptList of execution options
%
...
% to allow options try(prover_all_single) and try(prover_all):
% setlog(...,try(prover_all_single)) = setlog(...,try([s1,...,sn])), where s1,...,sn      
% are all singleton subsets of the set of options in 'all_prover_mode_strategies';
% setlog(...,try(prover_all)) = setlog(...,try([s1,...,sn])), where s1,...,sn      
% are all possible subsets of the set of options in 'all_prover_mode_strategies'
%
setlog1(Goal,TimeOut,OutConstrLst,Res,try(prover_all_single)) :- !,
    nb_getval(prover_all,[_,All]),
    singleton_lists(All,PS),
    setlog1(Goal,TimeOut,OutConstrLst,Res,try(PS)).
setlog1(Goal,TimeOut,OutConstrLst,Res,try(prover_all)) :- !,
    nb_getval(prover_all,[_,All]),   
    findall(S,setlog:powerset(All,S),PS),  % powerset/2 is defined in 'size_solver.pl'
    setlog1(Goal,TimeOut,OutConstrLst,Res,try(PS)).

% to allow options tryp(prover_all_single) and tryp(prover_all))
% where "p" means parallel, i.e. a thread is created for each singleton subset (resp.
% for each possi ble subset) of all_prover_mode_strategies. In other words, what in  
% tryp(prover_all_single) (resp., try(prover_all)) is executed sequentially, here 
% it's executed in parallel
%
setlog1(Goal,TimeOut,OutConstrLst,Res,tryp(prover_all_single)) :- !,  
    nb_getval(prover_all,[_,All]),   
    singleton_lists(All,PS),   
    setlog1(Goal,TimeOut,OutConstrLst,Res,tryp(PS)).
setlog1(Goal,TimeOut,OutConstrLst,Res,tryp(prover_all)) :- !,
    nb_getval(prover_all,[_,All]),   
    findall(S,setlog:powerset(All,S),PS),  
    setlog1(Goal,TimeOut,OutConstrLst,Res,tryp(PS)).
 
% to allow option try([OptsList_1,...,OptsList_n])
% e.g., setlog1(dom(R,S) & ran(R,{1/S}),2000,C,Res, try([[],[noran_elim]])).
% For each i ∈ 1..n−1, if the call:
%    setlog(Goal,Timeout,OutConstrList,OptsLst_i)
% terminates with a timeout, then the call:
%     setlog(Goal,Timeout,OutConstrList,Res,OptsLst_i+1)
% is attempted next; otherwise, the initial call terminates
% 
setlog1(Goal,TimeOut,OutConstrLst,Res,try([OptsLst|ROpts])) :- !, 
    set_default_control,  
    setlogTimeOut_opt(OptsLst,Goal,TimeOut,Constr,Res1),
    setlogTimeOut_cont(ROpts,Goal,TimeOut,Constr,OutConstrLst,Res1,Res).

% to allow option tryp([OptsList_1,...,OptsList_n])
% executes in parallel n goals of the form
%   rsetlog(Goal,TimeOut,OutConstrLst,Res,OptsLst_i)
% with i ∈ 1..n−1
%
setlog1(Goal,TimeOut,OutConstrLst,Res,tryp([OptsLst|ROpts])) :- !, 
    set_default_control,   
    mk_goal_list(Goal,TimeOut,OutConstrLst,Res,[OptsLst|ROpts],GoalList),
    term_variables(Goal,GoalVars),     
    ( first_solution([OutConstrLst,Res|GoalVars],GoalList,[on_fail(continue)]),!
    ;
      Res = time_out
    ).  

% with a not-empty list of options
% e.g., setlog1(X in int(1,5),1000,C,R,[int_solver(clpfd),nolabel]).
%
setlog1(Goal,TimeOut,OutConstrLst,Res,OptsLst) :- !,
    set_default_control,  
    setlogTimeOut_opt(OptsLst,Goal,TimeOut,OutConstrLst,Res).

% to execute 'Goal' with an output constraint list 'OutConstrList' and a 
% timeout 'TimeOut', with a not-empty list 'OptsLst' of execution options
%
...
% to execute/activate options in 'OptsList' not belonging to 'NoOptions'
%
...
% options in [Opt|OptsList] which are not in 'NoOptions' and are 
% setlog commands are immediately executed through ssolve_command/1;
% options in [Opt|OptsList] which are not in 'NoOptions' and belongs 
% to all_prover_mode_strategies are copied into 'ProverOptions'; they will
% be activated later within activate_opt/2
...
activate_opt1([Opt|OptsList],NoOptions,ProverOptions) :- 
    ...
    activate_opt1(OptsList,NoOptions,ProverOptions).
activate_opt1([Opt|OptsList],NoOptions,ProverOptions) :- 
    check_strategies([Opt]),           %true if [Opt] is a subset of all possible prover mode strategies
    \+ member_strong(Opt,NoOptions),!,
...
% to be used by setlog/5 when execution options contains the 'try' clause:
% setlogTimeOut_cont(+OptList,+Goal,+TimeOut,+InConstr,-OutConstr,+Res,-Return) 
% If either 'OptList' is empty or 'Res' is 'success' then simply binds 'Return' to 'Res';
% otherwise calls setlog1(...,try(OptList))
%
setlogTimeOut_cont([],_Goal,_TimeOut,Constr,Constr,Return,Return) :- !.
setlogTimeOut_cont(_,_Goal,_TimeOut,Constr,Constr,success,success) :- !.  % success
setlogTimeOut_cont(ROpt,Goal,TimeOut,_,OutConstrLst,_,Res) :-   % time_out
    setlog1(Goal,TimeOut,OutConstrLst,Res,try(ROpt)).

% check_timeout_res(+R,+SR,?Res)
% returns either 'success' or 'maybe' or 'timeout' depending on 'R' and 'SR'
%
...
% calls 'G' with time out 'T' msec and returns either 'success' or 'time_out' in 'R'; 
% if 'T' < 0, then raises an exception
%
...
mk_goal_list(_,_,_,_,[],[]) :- !.
mk_goal_list(Goal,TimeOut,OutConstrLst,Res,[OptsLst|ROpts],GoalList) :-
    GoalList = [(rsetlogTimeOut_opt(OptsLst,Goal,TimeOut,OutConstrLst,Res), Res \== time_out) | GoalList1],  
    mk_goal_list(Goal,TimeOut,OutConstrLst,Res,ROpts,GoalList1).

rsetlogTimeOut_opt(OptsLst,Goal,TimeOut,OutConstrLst,Res) :-
    set_default,
    nb_setval(sol,no),
    setlogTimeOut_opt(OptsLst,Goal,TimeOut,OutConstrLst,Res),
    nb_setval(sol,yes).
rsetlogTimeOut_opt(_OptsLst,_Goal,_TimeOut,_OutConstrLst,failure) :-
    nb_getval(sol,no).

check_strategies([]) :- !.             % [X|L] is a subset of all possible prover mode strategies
check_strategies([X|L]) :-
...
setlog1_str(GoalString,VarNames_or_Eqs,OutConstr,Res) :- 
  ...
  ;  
     ...
     extract_vars(VarNames_ext1,Vars),       
         % same as in prepare_answer
     term_variables(VarNames_ext1,V1),
     term_variables(Constr_ext,V2),
     append(V1,V2,AllVars),
     append(Vars,ConstrVars,NLVars),
     ord_subtract(AllVars,NLVars,LocVars),
     fresh_nl_variable_prefix(PrfNL),
...
setlog_help  :-
    (help_on ->
                  setlog_help(setlog)
    ;
        write('No help sub-system loaded')
    ).
...
set_initial_values :-
    ...
    nb_setval(solve_delay,false).    % set initial value: no elimination of delay atoms
/*
...
% (5) no constraints, only equalities. there might be
%     variables at the r.h.s. eg. A_ = {X/A} & A = {}.
sat_groundsol([],_) :-                     % (5)
  !,
  b_getval(varnames,VN),                   % to get variables at the r.h.s. of equalities
  extract_vars(VN,EqVar),                  % EqVar: only non-local variables
  (EqVar \== [] ->
    term_variables(EqVar,EqVar1),          % avoid repetitions
    (nb_getval(type_check,off) ->
       gen_other_eq(EqVar1,[],n,0,EqOther)
    ;
       gen_typed_eq(EqVar1,[],VN,n,0,EqOther)  % (4)
    ),
    sat(EqOther,_,f)                       % ground all non-integer variables
  ;
    true
  ).
sat_groundsol(SFC,SFCFinal) :-
...
%   non integer(_) constraints at this point: neq, npair, ninteger, nset
...
get_neq_int([npair(X)|SFCNew],[X|NeqVar],NeqConst,IntVar) :- !, 
    get_neq_int(SFCNew,NeqVar,NeqConst,IntVar).
get_neq_int([nset(X)|SFCNew],[X|NeqVar],NeqConst,IntVar) :- !, 
    get_neq_int(SFCNew,NeqVar,NeqConst,IntVar).
...
selected_atomic_constr(A) :-
    nb_getval(strategy,cfirst),!,        %% cfirst: select primitive constraints first 
    nonvar(A),atomic_constr(A).
selected_atomic_constr(A) :-
    nb_getval(strategy,cfirst(_)),!,    
    nonvar(A),atomic_constr(A).
selected_atomic_constr(A) :-
    nb_getval(strategy,ordered),!,       %% ordered: select all atoms in the order they occur
    nonvar(A),A = (_ = _).

selected_user_atom(A) :-
    nb_getval(strategy,cfirst(LAtoms)),   %Gselect atoms in LAtoms just after primitive constraints   
...
meta_pred(call(_)) :- !.
meta_pred(call(_,_)) :- !.
%meta_pred(time_out(_,_,_)) :- !.
%meta_pred(psolve_with_opts(_)) :- !.
meta_pred(t_solve(_)) :- !.
meta_pred(t_solve(_,_,_)) :- !.
meta_pred(p_t_solve(_)) :- !.
meta_pred(solve(_)) :- !.
meta_pred((_)!) :- !.
...
/*                                        
ssolve_meta(time_out(G,T,Res),C_out) :- !,  %% call with time_out  
    setlog_time_out(solve_goal_fin(G,C),T,Res),
    (Res == time_out -> 
         nb_setval(timeout_finish,true),
         C_out = []   
    ;                                
         C_out = C                        
    ).
...
% t_solve(G,T,Opts): "timed solve" - executes a {log} goal G with 
% a timeout T and a (possibly empty) list Opts of execution options;
%
ssolve_meta(t_solve(G,T,Opts),C_out) :- !,
    setlog_time_out_opts(G,T,C,Res,Opts),
    (Res == time_out -> 
        nb_setval(timeout_finish,true),
        C_out = []
    ; 
...
% t_solve(G): same as calling t_solve(G,T,[]) with default value for T
%
ssolve_meta(t_solve(G),C_out) :- !, 
    nb_getval(timeout,T),
    setlog_time_out(solve_goal_fin(G,C),T,Res),
    (Res == time_out -> 
        nb_setval(timeout_finish,true),
...
% p_t_solve(G): same as calling t_solve(G,T,tryp(PS)) with default  
% values for T and PS
%
ssolve_meta(p_t_solve(G),C_out) :- !,
    nb_getval(timeout,T),
    nb_getval(prover_all,[PA,All]),  
    (PA == all_single,!,        
     singleton_lists(All,PS)      
    ;
     PA == all,                 
    ...
    setlog_time_out_opts(G,T,C,Res,tryp(PS)),
    (Res == time_out -> 
        nb_setval(timeout_finish,true),
        C_out = []      
    ; 
...
ssolve_meta(solve(A),C) :- !,               %% forces goal A to be completely solved 
    b_getval(smode,Current),                %% (using the 'solver' mode)
...
setlog_time_out_opts(G,T,C,Res,[]) :- !,   
    setlog_time_out(solve_goal_fin(G,C),T,Res).
setlog_time_out_opts(G,T,C,Res,try(prover_all_single)) :- !,    
    nb_getval(prover_all,[_,All]),       
    singleton_lists(All,PS),   
    setlog_time_out_opts(G,T,C,Res,try(PS)).
setlog_time_out_opts(G,T,C,Res,tryp(prover_all_single)) :- !,    
    nb_getval(prover_all,[_,All]),       
    singleton_lists(All,PS),   
    setlog_time_out_opts(G,T,C,Res,tryp(PS)).
setlog_time_out_opts(G,T,C,Res,try(prover_all)) :- !,
    nb_getval(prover_all,[_,All]),       
    findall(S,setlog:powerset(All,S),PS),  
    setlog_time_out_opts(G,T,C,Res,try(PS)).
setlog_time_out_opts(G,T,C,Res,tryp(prover_all)) :- !,
    nb_getval(prover_all,[_,All]),       
    findall(S,setlog:powerset(All,S),PS),  
    setlog_time_out_opts(G,T,C,Res,tryp(PS)).
setlog_time_out_opts(G,T,C,Res,try([OptsLst|ROpts])) :- !, 
    solve_with_opts(OptsLst,G,T,C1,Res1),
    solve_with_opts_cont(ROpts,G,T,C1,C,Res1,Res).
setlog_time_out_opts(G,T,C,Res,tryp([OptsLst|ROpts])) :- !, 
    t_solve_mk_goal_list(G,T,C,Res,[OptsLst|ROpts],GoalList),   
    term_variables(G,GoalVars),     
    ( first_solution([C,Res|GoalVars],GoalList,[on_fail(continue)]),!
    ;
      Res = time_out    
    ),
    Res \== failure.
setlog_time_out_opts(G,T,C,Res,OptsLst) :- !, 
    solve_with_opts(OptsLst,G,T,C,Res).

solve_with_opts(OptsLst,G,T,C,Res) :- 
    activate_opt(OptsLst,[trace,groundsol,type_check]),
    setlog_time_out(solve_goal_fin(G,C),T,Res).

solve_with_opts_cont([],_,_,C,C,Return,Return) :- !.
solve_with_opts_cont(_,_,_,C,C,success,success) :- !.  % success
solve_with_opts_cont(ROpt,G,T,_,C,_,Res) :-   % time_out
    setlog_time_out_opts(G,T,C,Res,try(ROpt)).

t_solve_mk_goal_list(_,_,_,_,[],[]) :- !.
t_solve_mk_goal_list(Goal,TimeOut,OutConstrLst,Res,[OptsLst|ROpts],GoalList) :-
    GoalList = [(rt_solve_opt(OptsLst,Goal,TimeOut,OutConstrLst,Res), Res \== time_out) | GoalList1],  
    t_solve_mk_goal_list(Goal,TimeOut,OutConstrLst,Res,ROpts,GoalList1).

rt_solve_opt(OptsLst,Goal,TimeOut,OutConstrLst,Res) :-
    set_default,   
    nb_setval(sol,no),
    t_solve_opt(OptsLst,Goal,TimeOut,OutConstrLst,Res),
    nb_setval(sol,yes).
rt_solve_opt(_OptsLst,_Goal,_TimeOut,_OutConstrLst,failure) :-
    nb_getval(sol,no).

t_solve_opt(OptsLst,Goal,TimeOut,Constr,Res) :- 
    activate_opt(OptsLst,[trace,groundsol,type_check]),
    setlog_time_out(setlog2(Goal,Constr,Res1),TimeOut,Res2),   
    check_timeout_res(Res2,Res1,Res).
...
ssolve_command(strategy(Str)) :-     %% get the current atom selection strategy   
    var(Str),!,
    b_getval(strategy,Str).
ssolve_command(strategy(Str)) :- !,  %% change goal atom selection strategy
    (Str == cfirst -> true ; Str == cfirst(_) -> true ; Str == ordered),
    b_setval(strategy,Str).   

ssolve_command(prover_all_strategies(StrList)) :-         %% get the current prover_all strategy list 
    var(StrList),!,
    b_getval(prover_all,StrList).
ssolve_command(prover_all_strategies(all)) :- !,          %% restore the default value of prover_all strategy list
    all_prover_mode_strategies(PMS),     
    nb_setval(prover_all,[all,PMS]).           
ssolve_command(prover_all_strategies(all_single)) :- !,   %% list of singleton lists with all the options in prover_all_strategy
    all_prover_mode_strategies(PMS), 
    nb_setval(prover_all,[all_single,PMS]).          
ssolve_command(prover_all_strategies([PA,StrList])) :- !, %% change the prover_all strategy list
    member(PA,[all,all_single]),
    check_strategies(StrList),
    b_setval(prover_all,[PA,StrList]).   
...
ssolve_command(mode(M)) :- !,        %% change the constraint solving mode 
    ...
    ;
       M = prover(Lst),    
       check_strategies(Lst),
       b_setval(smode,M)    
    ).

ssolve_command(timeout(T)) :-        %% get the current timeout
    var(T),!,
    b_getval(timeout,T).
ssolve_command(timeout(T)) :- !,     %% change timeout 
    integer(T),
    0 < T,
    b_setval(timeout,T).
...
ssolve_special(consult(File),[]) :-        %% stores clauses contained in file F into
    ...
    ssolve_special(abolish,_),
       % These global variables are intended to be used by {log} components
       % such as VCG and TTF after typechecking has been performed. These
       % variables mantain a persistent database of the types used by the
       % program.
       %
       % [pred,varname,type] for all vars with dec(varname,type); pred=head/arity
    b_setval(typed_vars,[]),
       % [varname,type] for all vars with dec(varname,type) & has_finite(type)
    nb_setval(finite_vars,[]),
       % [varname,type] for all vars with dec(varname,type)&(type = sum(_) or type = enum(_))
    nb_setval(sum_vars,[]),
...
% user-defined predicates, with user-defined negative counterparts
%
negate(A,NegA) :-                       
...
% user-defined predicates, without user-defined negative counterparts but
% without existential variables, defined in a single clause whose arguments 
% are all variables
% (the previous case is executed first because if the
% user predicate has existential variables, then
% the user probably implemented the negative counterpart
% so we use the predicate defined by the user)
%
negate(A,NegA) :-
  nonvar(A),
  findall(_,isetlog((A :- _),_),[_]),     % only one clause defines A  
  isetlog((A :- B),_),
  A =.. [_|Params],                                                      
  forall(member(P,Params),var(P)),        % all A's arguments are variables  
  term_variables(A,VarA),
  extract_vars(B,NonLocalVars),           % local vars aren't existential
  forall(member(X,NonLocalVars),          % no existential variables in B (1)
         clpq:memberchk_eq(X,VarA)),!,
  preproc(neg(B),NB,C),
  list_to_conj(FC,C),
  NegA = (NB & FC).
%
% (1) no existential variables in B iff every non-local
%     variable in B is a variable in the head of B (i.e. A)

% user-defined predicates, without user-defined negative counterparts
%
negate(A, (naf A)).                     
...
load_ttf :-
    ...
    ;
        cond_retract(ttf),    
        write('No TTF implementation loaded (file '''),
        write(F), write(''')'), nl
    ).

%%%%%%%% load the help sub-system %%%%%%%%%%%%%
...
load_help :-
    ...
    (exists_file(FullName) ->
        ...
        assertz(help_on)
    ;
        cond_retract(help_on),    
        write('No help sub-system loaded (file '''),
        write(F), write(''')'), nl
    ).
...
setlog_read(Stream,Clause) :-
    \+type_checker,!, 
    nb_setval(err,false),                          
    setlog_read_term(Stream,Term,VarNames), 
    (Term = (:- Goal) ->  
     remove_dec(Term,Clause),  
     extract_vars(Goal,NonLocalVars),   
     remove_local(VarNames,NonLocalVars,VarNamesNL),    
     b_setval(varnames,VarNamesNL)     % varnames is used by sat_groundsol     
    ;
     Clause = Term
...
sat_eq_ris([T1 = S|R1],R2,c,F) :-         % ris(V,D,F,P) = {...}   (var-ris D)
    ...
    %
    Z = X,
    sat_step([FlNewD, PPNewD, Dom=D with CtrlExprNew, set(D) | R3], R2aux, _, F),
...
%%%-all other terms (including extensional sets)
...
sat_in([T in I|R1],R2,R,F) :-                 % extensional set;  t in {...}
    nonvar(I),!,
...
sat_in_s([T in Aggr|R1],R2,c,F) :-            % t in {...,t,...}
    ...
    sat_step(R1,R2,_,F).
sat_in_s([T in Aggr|R1],R2,c,F) :-            % set (case i): t in {...}
    aggr_comps(Aggr,A,_R),
    sunify(A,T,C),
    append(C,R1,R3),
    sat_step(R3,R2,_,F).
sat_in_s([T in Aggr|R1],R2,c,F) :-            % set (case ii): t in {...}
...
sat_nin([T nin A|_R1],_R2,c,_F) :-           % t nin {...,t,...} --> false
    ...
    fail.
sat_nin([T nin A|R1],R2,c,F) :-              % ground set/interval:
...
%%%%%%%% Under development - to be completed and tested (TODO)
%%%%%%%%%%%%%%%%%%%%% set sum (sum/2)
...
% deterministic membership (for intervals/sets):
...
sat_ran([ran(S,D)|R1],R2,c,F) :-                  % ran(r,d), general rule 
    b_getval(smode,prover(Strategies)),   % and the solving mode is prover([ran_fe,...])          
    member(ran_fe,Strategies),!,
    sat_step([foreach([X,Y] in S, [], Y in D, true),
              foreach(Y in D, [], exists([X,Z] in S, [], Y = Z, true), true)
             | R1],R2,_,F).
...
sat_oplus([oplus(R,S,T)|R1],R2,c,F) :-            % oplus(R,S,T)
    b_getval(smode,prover(Strategies)),   % and the solving mode is prover([oplus_fe,...])          
    member(oplus_fe,Strategies),!,
    sat_step([dom(S,D),set(D),
              foreach([X,Y] in T, [], [X,Y] in R & X nin D or [X,Y] in S, true),
              foreach([X,Y] in R, [], X nin D implies [X,Y] in T, true),
              foreach([X,Y] in S, [], [X,Y] in T, true) | R1],R2,_,F).
sat_oplus([oplus(R,S,T)|R1],R2,c,F) :-            % oplus(R,S,T)
...
global_check1([C|RC],GC,NewC,f) :-       % When groundsol is 'on', if C is W neq {}, var(W), and 
    nb_getval(groundsol,on),             % GC contains set(W) (or rel(W) or pfun(W))
    \+ noneq_elim,                       % then replace W neq {} with Z in W                     
    is_neq(C,1,W,T),                     
    var(W), nonvar(T), is_empty(T),
    find_set_type(W,GC),!,  
    trace_irules('neq_elim_groundsol'),
    mk_new_constr2(W,T,OutC),
    append(OutC,CC,NewC),
    global_check1(RC,GC,CC,f).

global_check1([C|RC],GC,NewC,f) :-       % C is W neq T, var(W) and var(T), and W or T are set variables
    \+ noneq_elim,                       % If GC contains W neq T & ... & Ck[W] or W neq T & ... & Ck[T]
    is_neq(C,1,W,T),                     % then replace W neq T with ((Z in W & Z nin T) or (Z nin W & Z in T)),
    var(W), var(T),                      % where Ck is either un/3 or subset/2 or inters/3 or diff/3 or size/2 or
    ...
    append(OutC,CC,NewC),
    global_check1(RC,GC,CC,f).
global_check1([C|RC],GC,NewC,f) :-       % C is W neq {...}, var(W), and W is a set variable
    \+ noneq_elim,                       % If GC contains W neq {} & ... & Ck[W] then replace W neq {} with Z in W
    is_neq(C,1,W,T),                     % If GC contains W neq {...} & ... & Ck[W] then replace W neq {...} with
    var(W), aggr_term(T),                %        ((Z in W & Z nin {...}) or (Z nin W & Z in {...})),
    find_setconstraint(W,GC),!,          % where Ck is as above
...
global_check2(ran(_,_),C,GC,AddedC,F) :-           % ran-neq
...
%%%%%% 
% find_setconstraint(+X,+GC) true if the (global) constraint store GC
% contains a set constraint Ck in solved form where X is: (i) one of its arg's,
% or (ii) the domain of a variable-RIS occurring as one of its arg's,  
% or (iii) one of the components of a variable-CP occurring as one of its arg's,  
% and Ck is either un/3 or subset/2 or inters/3 or diff/3 or size/2 or
% dom/2 or ran/2 or comp/3 or inv/2 or id/2 or ris = {} or ris = ris

find_setconstraint(X,cs(_,[C|_])) :-      % C is un/3 and X occurs in one of its three arg's
...
find_setconstraint(X,cs(_,[C|_])) :-
    is_sub(C,_,S1,S2),                    % C is subset/2 and X occurs in one of its two arg's
    var_ris_st(S1,S1Var), var_ris_st(S2,S2Var),
    (X == S1Var,! ; X == S2Var),!.
find_setconstraint(X,cs(_,[C|_])) :-
    is_sub(C,_,S1,_S2),                   % C is subset/2 and X occurs in its 1st arg.
    var_ris_st(S1,S1Var),
    X == S1Var,!.
find_setconstraint(X,cs(_,[C|_])) :-
    is_inters(C,_,S1,_S2,S3),             % C is inters/3 and X occurs either in its 1st or 3rd arg
    var_ris_st(S1,S1Var), var_ris_st(S3,S3Var),
    (X == S1Var,! ; X == S3Var),!.
find_setconstraint(X,cs(_,[C|_])) :-
    is_inters(C,_,_S1,S2,S3),             % C is inters/3 and X occurs either in its 2nd or 3rd argS
    var_ris_st(S2,S2Var), var_ris_st(S3,S3Var),
    (X == S2Var,! ; X == S3Var),!.
find_setconstraint(X,cs(_,[C|_])) :-      % C is size/2 and X occurs in its 1st arg.
    is_size(C,_,S1,_),
    var_ris_st(S1,S1Var),
    X == S1Var,!.
find_setconstraint(X,cs(_,[C|_])) :-
    is_diff(C,_,S1,S2,S3),                % C is diff/3 and either X or Y occurs in one of its three arg's
    var_ris_st(S1,S1Var), var_ris_st(S2,S2Var), var_ris_st(S3,S3Var),
    (X == S1Var,! ; X == S2Var,! ; X == S3Var),!.
find_setconstraint(X,cs(_,[C|_])) :-
    is_dom_l(C,_,S1,S2,_),                % C is dom/2 or dompf/2 and X occurs in one of its two arg's
    var_ris_st(S1,S1Var), var_ris_st(S2,S2Var),
    (X == S1Var,! ; X == S2Var),!.
find_setconstraint(X,cs(_,[C|_])) :-
    is_ran_l(C,_,S1,_S2),                 % C is ran/2 and X occurs in its 1st arg
    noran_elim,
    var_ris_st(S1,S1Var),
    X == S1Var,!.
find_setconstraint(X,cs(_,[C|_])) :-
    is_ran_l(C,_,S1,S2),                  % C is ran/2 and X occurs in one of its two arg's
    var_ris_st(S1,S1Var), var_ris_st(S2,S2Var),
    (X == S1Var,! ; X == S2Var),!.
find_setconstraint(X,cs(_,[C|_])) :-
    is_inv(C,_,S1,S2),                    % C is inv/2 and X occurs in one of its two arg's
    var_ris_st(S1,S1Var), var_ris_st(S2,S2Var),
    (X == S1Var,! ; X == S2Var),!.
find_setconstraint(X,cs(_,[C|_])) :-
    is_id(C,_,S1,S2),                     % C is id/2 and X occurs in one of its two arg's
    var_ris_st(S1,S1Var), var_ris_st(S2,S2Var),
    (X == S1Var,! ; X == S2Var),!.
find_setconstraint(X,cs(_,[C|_])) :-
    is_comp(C,_,S1,_S2,S3),               % C is comp/3 and X occurs either in its 1st or 3rd arg.
    var_ris_st(S3,S3Var), var_ris_st(S1,S1Var),
    (X == S1Var,! ; X == S3Var),!.
find_setconstraint(X,cs(_,[C|_])) :-
    is_comp(C,_,_S1,S2,S3),               % C is comp/3 and X occurs either in its 2nd or 3rd arg.
    var_ris_st(S3,S3Var), var_ris_st(S2,S2Var),
    (X == S2Var,! ; X == S3Var),!.
find_setconstraint(X,cs(_,[C|_])) :-
    is_comp(C,_,S1,_S2,S3),               % C is comp(_,_,{}) and X occurs in its 1st arg
    is_empty(S3), var_ris_st(S1,S1Var),
    X == S1Var,!.
find_setconstraint(X,cs(_,[C|_])) :-
    is_comp(C,_,_S1,S2,S3),               % C is comp(_,_,{}) and X occurs in its 2nd arg
    is_empty(S3), var_ris_st(S2,S2Var),
    X == S2Var,!.
find_setconstraint(X,cs(_,[C|_])) :-
    has_ris(C,_,D),                       % C is ris(_,X,_,_,_) = {} or T nin ris(_,X,_,_,_), X var.,
    X == D,!.
find_setconstraint(X,cs(_,[C|_])) :-
    has_ris2(C,_,D1,D2),                  % C is ris(_,D1,_,_,_) = ris(_,D2,_,_,_), D1, D2 var's,
    (X == D1,! ; X == D2),!.              % and X is either D1 or D2
find_setconstraint(X,cs(Neqs,[_|R])) :-
    find_setconstraint(X,cs(Neqs,R)).

%%%%%%
% find_setconstraint(+X,+Y,+GC,-Z1,-Z2) true if the (global) constraint store GC
% contains a set constraint Ck in solved form where either X or Y is (i) one of its arg's,
% or (ii) the domain of a variable-RIS occurring as one of its arg's,  
% or (iii) one of the components of a variable-CP occurring as one of its arg's, 
% and Ck is either un/3 or subset/2 or inters/3 or diff/3 or size/2 or
% dom/2 or ran/2 or comp/3 or inv/2 or id/2 or ris = {} or ris = ris;
% Z1 is bound to that variable between X and Y which occurs in C and Z2 is bound to the other 

find_setconstraint(X,Y,cs(_,[C|_]),Z1,Z2) :-
    is_un(C,_,S1,S2,S3),                   % C is un/3 and either X or Y occurs in one of its three arg's
    var_ris_st(S1,S1Var), var_ris_st(S2,S2Var), var_ris_st(S3,S3Var),
    one_of3(X,Y,S1Var,S2Var,S3Var,Z1,Z2),!.
find_setconstraint(X,Y,cs(_,[C|_]),Z1,Z2) :-
    is_sub(C,_,S1,S2),                     % C is subset/2 and either X or Y occurs in one of its two arg's 
    var_ris_st(S1,S1Var), var_ris_st(S2,S2Var),
    one_of2(X,Y,S1Var,S2Var,Z1,Z2),!.
find_setconstraint(X,Y,cs(_,[C|_]),Z1,Z2) :-
    is_sub(C,_,S1,_S2),                    % C is subset/2 and either X or Y occurs in its 1st arg.
    var_ris_st(S1,S1Var),
    one_of1(X,Y,S1Var,Z1,Z2),!.
find_setconstraint(X,Y,cs(_,[C|_]),Z1,Z2) :-
    is_inters(C,_,S1,_S2,S3),              % C is inters/3 and either X or Y occurs either in its 1st or 3rd arg
    var_ris_st(S1,S1Var), var_ris_st(S3,S3Var),
    one_of2(X,Y,S1Var,S3Var,Z1,Z2),!.
find_setconstraint(X,Y,cs(_,[C|_]),Z1,Z2) :-
    is_inters(C,_,_S1,S2,S3),              % C is inters/3 and either X or Y occurs either in its 2nd or 3rd arg
    var_ris_st(S2,S2Var), var_ris_st(S3,S3Var),
    one_of2(X,Y,S2Var,S3Var,Z1,Z2),!.
find_setconstraint(X,Y,cs(_,[C|_]),Z1,Z2) :-  % C is size/2 and either X or Y occurs in its 1st arg.
    is_size(C,_,S1,_),
    var_ris_st(S1,S1Var),
    one_of1(X,Y,S1Var,Z1,Z2),!.
find_setconstraint(X,Y,cs(_,[C|_]),Z1,Z2) :-
    ...
    one_of3(X,Y,S1Var,S2Var,S3Var,Z1,Z2),!.
find_setconstraint(X,Y,cs(_,[C|_]),Z1,Z2) :-
    is_dom_l(C,_,S1,S2,_),                 % C is dom/2 or dompf/2 and either X or Y occurs in one of its two arg's
    var_ris_st(S1,S1Var), var_ris_st(S2,S2Var),
    one_of2(X,Y,S1Var,S2Var,Z1,Z2),!.
find_setconstraint(X,Y,cs(_,[C|_]),Z1,Z2) :-
    is_ran_l(C,_,S1,_S2),                  % C is ran/2 and either X or Y occurs in its 1st arg
    noran_elim,
    var_ris_st(S1,S1Var),
    one_of1(X,Y,S1Var,Z1,Z2),!.
find_setconstraint(X,Y,cs(_,[C|_]),Z1,Z2) :-
    is_ran_l(C,_,S1,S2),                   % C is ran/2 and either X or Y occurs in one of its two arg's
    var_ris_st(S1,S1Var), var_ris_st(S2,S2Var),
    one_of2(X,Y,S1Var,S2Var,Z1,Z2),!.
find_setconstraint(X,Y,cs(_,[C|_]),Z1,Z2) :-
    is_inv(C,_,S1,S2),                     % C is inv/2 and either X or Y occurs in one of its two arg's
    var_ris_st(S1,S1Var), var_ris_st(S2,S2Var),
    one_of2(X,Y,S1Var,S2Var,Z1,Z2),!.
find_setconstraint(X,Y,cs(_,[C|_]),Z1,Z2) :-
    is_id(C,_,S1,S2),                      % C is id/2 and either X or Y occurs in one of its two arg's
    var_ris_st(S1,S1Var), var_ris_st(S2,S2Var),
    one_of2(X,Y,S1Var,S2Var,Z1,Z2),!.
find_setconstraint(X,Y,cs(_,[C|_]),Z1,Z2) :-
    is_comp(C,_,S1,_S2,S3),                % C is comp/3 and either X or Y occurs either in its 1st or 3rd arg.
    var_ris_st(S3,S3Var), var_ris_st(S1,S1Var),
    one_of2(X,Y,S1Var,S3Var,Z1,Z2),!.
find_setconstraint(X,Y,cs(_,[C|_]),Z1,Z2) :-
    is_comp(C,_,_S1,S2,S3),                % C is comp/3 and either X or Y occurs either in its 2nd or 3rd arg
    var_ris_st(S3,S3Var), var_ris_st(S2,S2Var),
    one_of2(X,Y,S2Var,S3Var,Z1,Z2),!.
find_setconstraint(X,Y,cs(_,[C|_]),Z1,Z2) :-
    is_comp(C,_,S1,_S2,S3),                % C is comp(_,_,{}) and either X or Y occurs in its 1st arg
    is_empty(S3), var_ris_st(S1,S1Var),
    one_of1(X,Y,S1Var,Z1,Z2),!.
find_setconstraint(X,Y,cs(_,[C|_]),Z1,Z2) :-
    is_comp(C,_,_S1,S2,S3),                % C is comp(_,_,{}) and either X or Y occurs in its 2nd arg
    is_empty(S3), var_ris_st(S2,S2Var),
    one_of1(X,Y,S2Var,Z1,Z2),!.
find_setconstraint(X,Y,cs(_,[C|_]),Z1,Z2) :-
    has_ris(C,_,D),                        % C is ris(_,D,_,_,_) = {} or T nin ris(_,D,_,_,_), D var., 
    one_of1(X,Y,D,Z1,Z2),!.                % and either X or Y is D
find_setconstraint(X,Y,cs(_,[C|_]),Z1,Z2) :-
    has_ris2(C,_,D1,D2),                   % C is ris(_,D1,_,_,_) = ris(_,D2,_,_,_), D1, D2 var., 
    one_of2(X,Y,D1,D2,Z1,Z2),!.            % and either X or Y is D1 or D2
...
find_set_type(X,cs([C|_],_)) :-       
    set_type(C,S),
    X == S,!.
find_set_type(X,cs([_|R],Others)) :-
    find_set_type(X,cs(R,Others)).
...
set_type(set(S),S).                         % set type constraints   
set_type(rel(R),R).
set_type(pfun(F),F).

% no_type_error_all(+C,+CList): true if the list of constraints CList
% doesn't contain any contraint B which is incompatible with the constraint C
%
...
% check_type_error(+F1,+F2): F1 and F2 are the functors of two distinct
% type constraints (e.g. 'set' and 'nset'). check_type_error(F1,F2) is
% true F1 and F2 are "incompatible" according to the definition provided
% by predicate incompatible/2
%
check_type_error(F1,F2) :-
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% transform_goal/2
% Transform a goal/clause from the external representation to the
% internal representation by calling preproc_goal/preproc_clause
% and rewriting (general) intensional sets and RUQs
...
%%%%%%%%% preproc goals and clauses 
% Transform a goal/clause from the external representation to the
% internal representation by calling preproc/3, 
% removing possible duplicate elements from the list of the generated 
% type constraints
% e.g. preproc_goal(un(A,{[X,Y]/B},C) & disj(A,B),GInt).
%      GInt = un(A, B with [X, Y], C) & disj(A, B) & set(C) & set(A) & set(B) & true
...
%%%%%%%%% proproc terms
% preproc(+GExt,-GInt,-TypeC)
% Transform a term/formula from the external representation (GExt) to the
% internal representation (GInt) by rewriting set, interval, RIS, and CP terms,
% as well as conjunctions, disjunctions, clauses and atomic predicates
% (including foreach, exists, neg, meta-predicates, and user-defined predicates)
% Return in TypeC the list of generated sort constraints (possibly with repetitions)
% e.g. preproc(un(A,{[X,Y]/B},B),GInt,TypeC).
%      GInt = un(A, B with [X, Y], B),
%      TypeC = [set(B), set(A), set(B)].
...
preproc(neg A,neg(A1),C) :-     
    !, preproc(A,A1,C).        % preproc instead of preproc_goal to put sort constraints outside the negation
...
is_exists(exists(X,Fo),exists(X,Fo1),C) :-         % general exists
    var(X),!,
    is_exists(exists(X in Dummy,Fo),exists(_,Fo1),C1),
    remove_cons(C1,set(Dummy),C).
is_exists(exists(D,Fo),exists(D1,Fo1),C) :-
    nonvar(D), D = [_|_],!,                        % nested exists
    unfold_nested_exists(exists(D,Fo),exists(DNew,FoNew1)),
    is_foreach(foreach(DNew,[],FoNew1,true),foreach(D1,_,FoNew2,TypeC),C),
    conj_append(FoNew2,TypeC,Fo1).
is_exists(exists(D,Fo),exists(D1,Fo1),C) :-                                  
    is_foreach(foreach(D,[],Fo,true),foreach(D1,_,FoNew,TypeC),C),
    conj_append(FoNew,TypeC,Fo1).
...
remove_cons([C1|CList],Cons,CListNew) :-
    C1 == Cons,!,
    remove_cons(CList,Cons,CListNew).
remove_cons([C1|CList],Cons,[C1|CListNew]) :-
    remove_cons(CList,Cons,CListNew).

%%%%%%%%%%%%%%%%%% let preprocessing

is_let(let(LVars,Conj,F),let(LVars,ConjNew1,FNew),C1) :-
    ...
    preproc(F,FNew,C),
    separate_sortc_LV(C,LVars,C1,LV_SortConsList),
    list_to_conj(LV_SortConsConj,LV_SortConsList),
    conj_append(ConjNew,LV_SortConsConj,ConjNew1).
...
%%%%%%%%%%%%% auxiliary predicates for set preprocessing
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%% User-defined rewriting rules ("filtering rules") %%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%             rel_term/1,
%             pfun_term/1,
% List/Set/Interval/RIS/CP manipulation
%             aggr_term/1,
%             tail/2,
%             tail2/2,
%             same_tail/3,
...
singleton_lists([],[]) :- !.     %singleton_lists(L,NL): true if L is [e1,...,en] and NL is [[e1],...,[en]]
singleton_lists([X|L],[[X]|NL]) :-
   singleton_lists(L,NL).
...
%%% ttf file (default: 'setlog_ttf.pl')
ttf_file('setlog_ttf.pl').

%%% help file (default: 'setlog_help.pl')
help_file('setlog_help.pl').
...
all_prover_mode_strategies(
  [subset_unify,comp_fe,un_fe,oplus_fe,ran_fe,noirules,strategy(ordered)]). 

%%% the atom selection strategy (default: cfirst)
default_sel_strategy(cfirst).                 % "constraint first"
%default_sel_strategy(ordered).               % as they occur in the formula
%default_sel_strategy(cfirst(UserPredList)).  % UserPredList is a list of user defined predicates to be
                                  % executed just after constraint solution (before all other
                                  % user-defined predicates)
                                  % e.g., default_sel_strategy(cfirst([ttf_list(_),ttf_nat(_),ttf_int(_),ttf_btype(_)])).
...
%%% default timeout; used by t_solve/1 and p_t_solve/1
...
type_constraints_to_be_shown([set(_),integer(_),rel(_),pfun(_),pair(_),
...
% load the ttf implementation (if any)
:- initialization(load_ttf).

% load the help sub-system (if any)
:- initialization(load_help).

% print the initial message for interactive use
:- initialization((nl,write('Use ?- setlog_help or h(setlog) to get help information about {log}'),nl,nl)).
