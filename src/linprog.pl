...
% WE NEED MORE MEMORY!
% :- set_prolog_flag(stack_limit, 80000000). % 8MB stack limit, default is 4MB
...
%:- nb_setval(timeLimit, 1000). % time limit for GLPK, 1000s by default
...
linprog:attr_unify_hook(name_n_AllVars(XName, XOrdSet), Y) :-
	...
	% writeln('It is eq to Y with attrs:'),
        ...
        string_concat(Expr, '=0', Constraint),
        store_expr(Constraint),
        % checkAddingConstraint(Constraint),
    ...
    ;   (number(Y)
        ->  
            ...
            string_concat(Xeq, YS, Constraint),
            store_expr(Constraint)
...
% -----------------------------------Compress------------------------------------------------------------
/* Adding a pair of Var and Coef to list */
add(Var, A, B, C) :-
    var(Var),
    !,
    % writeln('Recive such a var' : Var),
     (get_attr(Var, linprog, name_n_AllVars(VarStr, _))
    -> add(VarStr, A, B, C)
    ;  writeln('ERROR -> [add]: unknown var'),
        writeln(Var), writeln(A), writeln(B), writeln(C) 
    ).
    
/* Takes a pair of Var and Coef and add it to the list of pairs <VarStr, Coef> */
add(Var, Coef, [], [[Var, Coef]]) :- 
        string(Var), number(Coef), 
        !.

add(Var, Coef, [H|Tail], [[Var, NewCoef]|Tail]) :- 
    string(Var), number(Coef), 
    member(Var, H),
    !,   
    nth0(1, H, X),
...
add(Var, Coef, [H|Tail], [HNew| NewTail]) :- 
    string(Var), number(Coef), 
    \+ member(Var, H),
    !,
    HNew = (H),
    % writeln('Var is not a member H is ' : H),
    add(Var, Coef, Tail, NewTail).

add(V, C, L, _) :-
    writeln('ERROR -> [add]: unkown vars'),
    writeln(V), writeln(C), writeln(L).

un([], L2, Res) :- 
    !,
    Res = L2. 

un([[Var, Coef]| Tail], L2, Res) :- 
    string(Var), number(Coef), 
    !,
    % writeln('Getting to add with'),
    % writeln(Var), writeln(Coef), 
    add(Var, Coef, L2, L2new), 
    un(Tail, L2new, Res).

un(A, B, _) :- 
    writeln('ERROE -> un -> cannot unit '),
    writeln(A), writeln(B).
...
compress(A*B, Res, Num, IsLMneg) :- 
    number(A), var(B), 
    !,
    % writeln('Find A*B, where var is B' : B),
    Cmp is IsLMneg * A, 
    add(B, Cmp, [], Res),
    Num is 0.
...
compress(A*B, Res, Num, IsLMneg) :- 
    var(A), number(B), 
    !,
    % writeln('Find A*B, where var is A' : A),
    add(A, IsLMneg*B, [], Res),
    Num is 0.
...
bb_inf_b(IntsOrig, ObjOrig, Inf, Vertex, Eps, St) :- 
    ...
    % write('IntsOrig ' : IntsOrig), write('ObjOrig' : ObjOrig),
    % write('Inf ' : Inf), write('Vertex ' : Vertex), nl, nl,
    base_check(IntsOrig),
    get_exprs(Cons),
    prepareIntsToBB_INF(IntsOrig, Ints),
    ...
    b_getval(timeLimit, TimeLimit),
    b_getval(solver, Solver),
    call(Solver, Cons, Ints, Obj, InfWithoutConst, Vertex, St, Eps, 1, TimeLimit),
